...
**不可逆的影響が回避不能になる前に、実効的な人間拒否権は利用可能で、実行可能で、検証可能だったか。**
...
- 人間の意思決定者、レビュー担当者、統治主体、または責任組織を特定できる。
- 拒否権が名目的なものではない。
...
- 拒否、停止、延期、上書き、封じ込め、方向修正のいずれかを時間内に実行できた。
...
必要条件のいずれかを確認できない場合、LUMINA-30境界レビュー上は「実効的な人間拒否を示していない」と分類する。
...
これは法的責任、意図、技術欠陥を証明するものではない。境界レビュー条件が示されなかったことのみを意味する。
...
本ファイルは、最初の3ファイルと併用する判定基準の集約ページであり、最初の3ファイルを置き換えるものではない。

- [LUMINA-30 Boundary Check](./LUMINA-30_Boundary_Check.md)  
  Use for the central boundary question.  
  中核境界問いの確認に使用する。

- [Incident Quick Assessment Template](./Incident_Quick_Assessment_Template.md)  
  Use for minimum fact collection.  
  最小事実収集に使用する。
